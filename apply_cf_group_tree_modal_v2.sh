...
CSS_FILE="$(pick_file "$ROOT" \
  ...
  )" || { echo "[ERR] application.css(.scss/.sass) not found"; exit 1; }
...
echo "--- Patching JS (VSCode tree collapse + group create/edit modal with parent select) ---"
export CF_JS_FILE="$JS_FILE"
python3 - <<'PY'
...
js_path = Path(os.environ["CF_JS_FILE"])
src = js_path.read_text(encoding="utf-8")

MARK = "CF_GROUP_TREE_MODAL_V2"
if MARK in src:
    print("JS already patched (marker found), skip")
else:
    # Find the renderGroupTree function and inject our block right after its closing brace
  ...
  // === CF_GROUP_TREE_MODAL_V2 ===
  // VSCode-like group collapse + group create/edit modal (parent selectable)
  const CF_GROUP_TREE_STORAGE_KEY = 'cf:groupTreeCollapsed:v2';
  ...
  async function cfOpenGroupModal({ mode: formMode, group = null, parentId = null } = {}) {
    ...
    nameEl.value = isEdit ? (editingGroup && editingGroup.name ? editingGroup.name : '') : '';
    ...
    const selected = isEdit
      ? (editingGroup && editingGroup.parent_id ? String(editingGroup.parent_id) : '')
    ...
    const handler = async () => {
      ...
      try {
        ...
        const createdId = (!isEdit && result && ( (result.group && result.group.id) || result.id ))
          ? Number((result.group && result.group.id) || result.id)
          : null;
    ...
    setTimeout(() => { try { nameEl.focus(); } catch(e){} }, 0);
  }
  ...
  // Make sure initial render uses VSCode style
  try { renderGroupTree(); } catch (e) {}
  // === END CF_GROUP_TREE_MODAL_V2 ===
"""

    patched = src[:func_end + 1] + inject + src[func_end + 1:]
    js_path.write_text(patched, encoding="utf-8")
    print("Injected CF_GROUP_TREE_MODAL_V2 block")
PY

echo "--- Patching CSS (tree controls UI) ---"
export CF_CSS_FILE="$CSS_FILE"
python3 - <<'PY'
from __future__ import annotations
...
css_path = Path(os.environ["CF_CSS_FILE"])
src = css_path.read_text(encoding="utf-8")

MARK = "CF_GROUP_TREE_VSCODE_UI_V2"
if MARK in src:
    print("CSS already patched (marker found), skip")
else:
    block = """

/* === CF_GROUP_TREE_VSCODE_UI_V2 === */
...
#cf-group-modal.hidden { display: none; }
#cf-group-modal.cf-modal { position: fixed; inset: 0; z-index: 9999; }
#cf-group-modal .cf-modal-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.55); }
...
#cf-group-modal .cf-modal-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  margin-bottom: 10px;
}
#cf-group-modal .cf-field label { display: block; font-size: 12px; opacity: 0.85; margin-bottom: 6px; }
#cf-group-modal .cf-field input,
...
#cf-group-modal .cf-field { margin-bottom: 10px; }
#cf-group-modal .cf-modal-actions { display: flex; justify-content: flex-end; }
#cf-group-modal .cf-muted { opacity: 0.75; font-size: 12px; }
/* === END CF_GROUP_TREE_VSCODE_UI_V2 === */
"""
    css_path.write_text(src + block, encoding="utf-8")
    print("Appended CF_GROUP_TREE_VSCODE_UI_V2 block")
PY
