...
class AiClientSeptemberEvalRegressionTest < ActiveSupport::TestCase
  EVALUATION = JSON.parse(File.read(Rails.root.join('test/fixtures/ai_september_eval.json'))).freeze
  ...
  test 'evaluation fixture preserves all submitted originals and the full long input' do
    assert_equal 15, INPUTS.length
    assert_equal 1000, INPUTS.fetch('CF-07b').length
    assert_equal '2026-09-14T21:57:59+09:00', CONTEXT.fetch(:now)
    assert_equal 'Asia/Tokyo', CONTEXT.fetch(:timezone)
    assert INPUTS.fetch('CF-07b').start_with?('2026年9月17日の15:00から15:30まで、架空の「CF-07b テスト長文集中作業」')
  ...
  test 'CF-01 keeps the explicit title and successful tomorrow time range' do
    ...
    assert_event(response.fetch(:recommendations).sole,
                 title: 'CF-01 テスト集中作業',
                 start_at: '2026-09-15T15:00:00+09:00', end_at: '2026-09-15T15:30:00+09:00')
  end
  ...
  test 'CF-02 separates two schedules and preserves the future schedule while identifying the past one' do
    ...
    if remaining.any?
      assert_event(remaining.sole, title: 'CF-02 テスト休憩',
                   start_at: '2026-09-14T18:00:00+09:00', end_at: '2026-09-14T18:30:00+09:00')
    else
      assert_includes response.fetch(:assistant_message), 'CF-02 テスト休憩'
    end
  end

  test 'CF-03 either asks for missing timing or explains assumptions within next week' do
  ...
  test 'CF-04b continues to reject invalid clocks and preserves minutes in the correction example' do
    ...
    assert_match(/翌日1時30分|翌(?:日)?\s*0?1:30/, response.fetch(:assistant_message))
  end
  ...
  %w[CF-05 CF-05R].each do |case_id|
    test "#{case_id} interprets mixed language timing and supplementary candidate instructions" do
      ...
      assert_event(response.fetch(:recommendations).sole, title: "#{case_id} テスト休憩",
                   start_at: '2026-09-15T14:00:00+09:00', end_at: '2026-09-15T14:20:00+09:00')
  ...
  test 'CF-06 preserves the weekly title and shows the range of all sixteen occurrences' do
    ...
    expected_dates = 8.times.flat_map do |week|
      [Date.new(2026, 9, 15) + week * 7, Date.new(2026, 9, 17) + week * 7]
    ...
    assert_match(/9[\/月]15|2026-09-15/, message)
    assert_match(/11[\/月]5|2026-11-05/, message)
  ...
  test 'CF-07b keeps the explicit title and successful time range in the full thousand character input' do
    ...
    assert_event(response.fetch(:recommendations).sole, title: 'CF-07b テスト長文集中作業',
                 start_at: '2026-09-17T15:00:00+09:00', end_at: '2026-09-17T15:30:00+09:00')
  end

  test 'CF-08 keeps one candidate and the requested time range at the service boundary' do
    response = response_for('CF-08')

    assert_equal 1, response.fetch(:recommendations).length
    assert_event(response.fetch(:recommendations).sole, title: 'CF-08 テスト集中作業',
                 start_at: '2026-09-18T16:00:00+09:00', end_at: '2026-09-18T16:30:00+09:00')
  end
  ...
  test 'CF-02b resolves next Tuesday in the next calendar week' do
    response = response_for('CF-02b')

    assert_equal 1, response.fetch(:recommendations).length
    assert_event(response.fetch(:recommendations).sole, title: 'CF-02b テスト集中作業',
                 start_at: '2026-09-22T10:00:00+09:00', end_at: '2026-09-22T11:00:00+09:00')
  end
end
