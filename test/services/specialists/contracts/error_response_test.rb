...
class SpecialistsContractsErrorResponseTest < ActiveSupport::TestCase
  ERROR_CASES = {
    'malformed_json' => [400, 'Request body is not valid JSON.', false],
    'invalid_token' => [401, 'Authentication is required.', false],
    'expired_token' => [401, 'Authentication has expired.', false],
    'replayed_token' => [401, 'Authentication could not be accepted.', false],
    'insufficient_scope' => [403, 'Permission is insufficient.', false],
    'inactive_user' => [403, 'The user account is inactive.', false],
    'inactive_membership' => [403, 'The membership is inactive.', false],
    'unsupported_media_type' => [415, 'The request media type is not supported.', false],
    'invalid_request_schema' => [422, 'Request does not match the required schema.', false],
    'unsupported_capability' => [422, 'The requested capability is not supported.', false],
    'invalid_response_schema' => [500, 'Specialist response validation failed.', false],
  ...
  test 'maps every canonical boundary error to a fixed schema-valid envelope' do
    ERROR_CASES.each do |code, (expected_status, expected_message, expected_retryable)|
      ...
      assert_equal expected_retryable, result.body.fetch('retryable'), code
