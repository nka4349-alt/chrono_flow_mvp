...
module Ai
  class Client
    ...
    def local_weekday_multi_event_response(text)
      ...
      items = split_event_clauses(text).filter_map do |clause|
        date = first_local_date_from_text(clause)
        next unless date && target_weekdays(clause).any?
...

        {
          ...
          descriptor: descriptor,
          complete: !insufficient_activity_title?(title) && title != '予定'
        }
      end
      ...
      incomplete_items = items.reject { |item| item[:complete] }
      if incomplete_items.any?
        missing_weekdays = incomplete_items.map { |item| WEEKDAY_LABELS.fetch(item[:date].wday) }.uniq.join('・')
        return {
    ...
    def local_single_explicit_event_response(text, require_explicit_time: false)
      ...
      explicit_duration = explicit_duration_minutes(text)
