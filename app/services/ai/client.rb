...
events = 8.times.map do |i|
    ...
    def schedule_event_clauses(text)
      split_event_clauses(text).select do |clause|
        next false if clause.blank?

        descriptor = local_event_descriptor(clause)
        title = clean_activity_title(descriptor[:activity_title].presence || descriptor[:title])
        next false if schedule_event_framing_clause?(clause, title)
    ...
    def explicit_duration_minutes_outside_clock_times(text)
      ...
      explicit_duration_minutes(
        text_outside_clock_matches(duration_source, explicit_time_matches(duration_source))
      )
    end
